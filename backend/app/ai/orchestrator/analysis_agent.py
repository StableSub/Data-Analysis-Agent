...
class AnalysisState(TypedDict):
    ...
    plan: list[dict]
    raw_response: str
    response: AnalysisCodeResponse


class AnalysisAgent:
    """
    데이터 분석을 위한 파이썬 코드를 생성하는 에이전트.
    ...
    def _draft_node(self, state: AnalysisState) -> dict:
    ...
    def _build_context(data_context: str, plan: Iterable[dict]) -> str:
        ...
        return (
            "역할: Analysis Agent\n"
            "- 사용자 질문에 대한 분석용 파이썬 코드를 생성한다.\n"
            "- 출력은 반드시 JSON 형식으로만 제공한다.\n"
            "- 코드는 pandas를 사용하며, 기본 데이터프레임 이름은 df로 가정한다.\n"
    ...
    @staticmethod
    def _parse_response(response: str, fallback_question: str) -> AnalysisCodeResponse:
        cleaned = AnalysisAgent._strip_code_fences(response)
        try:
            payload = json.loads(cleaned)
        except json.JSONDecodeError:
            return AnalysisCodeResponse(
                objective=fallback_question.strip() or "Analyze the data",
                code=(
                    "import pandas as pd\n\n"
                    "# TODO: add analysis steps based on the question\n"
    ...
    def _parse_node(self, state: AnalysisState) -> dict:
