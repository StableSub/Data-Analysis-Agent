...
def _build_llm(
    model: str, temperature: float, api_key_env: str
) -> ChatGoogleGenerativeAI:
    """필요한 환경변수를 검사해 ChatGoogleGenerativeAI 인스턴스를 생성한다."""
    api_key = os.getenv(api_key_env)
    if not api_key:
        raise RuntimeError(
            f"{model}을(를) 초기화하려면 환경변수 '{api_key_env}'가 필요합니다."
        )
...
def get_llm(preset: Optional[LLMPresetName] = None) -> ChatGoogleGenerativeAI:
    ...
    """
    preset_name = preset or os.getenv("LLM_PRESET", "gemini_flash")
    settings = LLM_PRESETS.get(preset_name)
    if not settings:
        available = ", ".join(sorted(LLM_PRESETS))
        raise ValueError(
            f"알 수 없는 LLM 프리셋 '{preset_name}'. 사용 가능: {available}"
        )
