...
PROMPTS = PromptRegistry(
    {
        ...
        "code_generation.system": (
            ...
            "입력 데이터는 dataset_path 변수와 이미 로드된 pandas DataFrame df로 주어진다. "
            ...
            "허용 라이브러리는 json, math, statistics, datetime, pandas, numpy 뿐이다. "
            "datetime import는 날짜 범위 계산이 정말 필요할 때만 사용하고, 가능하면 pandas의 to_datetime과 dt accessor를 우선 사용하라. "
            "os, sys, pathlib, subprocess, shutil, glob, pickle, importlib는 절대 import하지 마라. "
        ...
        "code_repair.system": (
            ...
            "month, year_month, ratio, temp_speed_ratio 같은 파생/임시/helper 컬럼은 used_columns에 넣지 마라. "
            "입력 데이터는 dataset_path 변수와 이미 로드된 pandas DataFrame df로 주어진다. "
            "datetime import는 허용되지만 날짜 범위 계산이 정말 필요할 때만 사용하고, 가능하면 pandas의 to_datetime과 dt accessor를 우선 사용하라. "
            "os, sys, pathlib, subprocess, shutil, glob, pickle, importlib는 절대 import하지 마라. "
            "추가 파일 읽기/쓰기, 환경 변수 접근, 프로세스 실행, 네트워크 호출은 금지한다. "
            "analysis_plan.time_context.relative_range_resolved가 있으면 start/end를 그대로 사용하고, now/today/current month를 다시 계산하지 마라. "
            "time_context.grain이 month이고 relative_expr가 last 3 months 계열이면, 현재 진행 중인 월은 제외하고 최근 3개 완료월만 사용하라. "
