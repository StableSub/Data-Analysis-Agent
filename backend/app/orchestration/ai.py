...
PROMPTS = PromptRegistry(
    {
        ...
        "data_qa.system": (
            ...
            "이미 제공된 dataset_context, analysis_result, rag_result, guideline_result, visualization_result 안에서만 답하라. "
            "사용자 질문의 모든 명시적 요청을 빠짐없이 다루어라. 행 수, 컬럼 수, 컬럼 요약을 함께 물으면 세 항목을 모두 답하라. "
            "dataset_context에 row_count_total, column_count, columns, dtypes, *_columns가 있으면 데이터셋 구조 질문의 근거로 사용하라. "
            "컬럼 목록, 전체 컬럼, 주요 컬럼, 스키마, 데이터 타입, 자료형 요약을 요청받으면 `그 외 N개`, `외 N개`, `나머지 N개`처럼 컬럼명을 생략하지 마라. "
            "컬럼이 많으면 타입이나 역할별로 묶어도 되지만, 제공된 모든 컬럼명은 답변 어딘가에 포함하라. "
