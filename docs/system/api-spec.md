...
| 그룹 | 용도 | 대표 사용 시점 | 특이사항 |
| --- | --- | --- | --- |
| `chats` | 질문 시작, 승인 대기, 재개, 세션 목록/히스토리 | 사용자가 대화형 분석을 시작하거나 기존 대화를 복원할 때 | SSE 스트리밍 사용 |
...
- 역할: 채팅 세션 목록 조회
- 언제 쓰는가: Workbench 초기 진입 시 서버에 남아 있는 세션을 sidebar에 복원할 때
- 핵심 query 파라미터:
  - `skip`
  - `limit`
- 핵심 응답 필드:
...
- 참고: DB row가 있어도 `storage_path` 파일이 없으면 목록에서 제외한다. `skip`/`limit`은 파일 존재 여부 필터링 이후 적용된다.
- 핵심 응답 필드:
  - `total`
  - `items`
