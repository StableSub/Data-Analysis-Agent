...
| `backend/app/modules/eda/nonexpert_guidance.py` | EDA payload에서 데이터셋 구조, 품질, 그룹/시간/이상치 신호를 읽어 `suggested_questions`를 만든다. |
...
- `GET /eda/{source_id}/insights`: AI 기반 EDA insight. 선택 query `guideline_source_id`가 있으면 해당 지침서를 우선 사용하고, 없으면 활성 guideline을 조회한다. 지침 문맥을 읽을 수 있으면 데이터 프로파일과 합친 `dataset_overview`를 함께 반환한다. 응답에는 `suggested_questions`도 포함되어 Workbench가 데이터셋 기반 시작 질문을 표시한다.
